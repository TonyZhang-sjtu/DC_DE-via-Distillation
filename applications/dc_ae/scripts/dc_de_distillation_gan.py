...
def train_distillation(args):
    ...
    scheduler_D = optim.lr_scheduler.CosineAnnealingWarmRestarts(
        optimizer_D,
        T_0=args.cosine_T_0_D,  
        T_mult=args.cosine_T_mult_D, 
        eta_min=args.eta_min_D,        
        verbose=True
    )
    ...
    for epoch in range(args.num_epochs):
        ...
        with tqdm(train_loader, desc=f"Epoch {epoch+1}/{args.num_epochs}", total=args.train_samples/args.batch_size, unit='img') as pbar:
            for images, _ in pbar:
                ...
                if i % args.gan_ratio == 0:
                    optimizer_D.zero_grad()
                    real_pred = discriminator(images.detach()) # Use ground truth images instead of teacher results
                    fake_pred = discriminator(student_img.detach())

                    valid = torch.ones_like(real_pred)
                    fake = torch.zeros_like(fake_pred)

                    loss_D_real = gan_loss_fn(real_pred, valid)
                    loss_D_fake = gan_loss_fn(fake_pred, fake)
                    loss_D = 0.5 * (loss_D_real + loss_D_fake)
                    loss_D.backward()

                    accumulated_batches_D += 1
                    if accumulated_batches_D >= accumulation_steps:
                        optimizer_D.step()
                        optimizer_D.zero_grad()
                        accumulated_batches_D = 0
                ...
                gan_pred = discriminator(student_img)
                gan_g_loss_img = gan_loss_fn(gan_pred, valid)  # student are expected to be real

                loss_G = alpha_disti * l1_dis + alpha_img * l1_img + beta * lpips_val_img + gamma * gan_g_loss_img
                loss_G.backward()
                ...
                gan_img_loss += gan_g_loss_img.item()
        ...
        avg_gan_img_loss = gan_img_loss / args.train_samples
        ...
        with open(f"{args.model_save_dir}/training_losses.txt", "a") as f:
            f.write(f"Epoch {epoch+1}/{args.num_epochs}, L1_Dis: {avg_l1_dis_loss:.4f}, L1_Img: {avg_l1_img_loss:.4f}, LPIPS_Img: {avg_lpips_val_loss:.4f}, GAN_Img: {avg_gan_img_loss.item():.4f}\n")
